import { Toast } from "@weimai/maiui-mobile";
import axios, {
  AxiosPromise,
  AxiosRequestConfig,
  AxiosInstance,
...
const setHeader = axiosInst.interceptors.request.use(
  async (config: TConfigExpandByAxiosRequestConfig) => {
    ...
    if (isWeimaiPatient) {
      ...
      if (query.token) {
        TOKEN_VALUE = query.token as string;
        sessionStorage.setItem('x-mini-weimai-token', query.token as string)
      } else {
        TOKEN_VALUE = sessionStorage.getItem('x-mini-weimai-token') || ''
      }
    } else {
...
axiosInst.interceptors.response.use(
  (response: TResponseExpandByAxiosResponse) => {
    ...
    if (headers[TOKEN_KEY]) {
      ...
      if (isWeimaiPatient && compare(version, "4.8.0") === 1) {
  ...
  (err: Error) => {
    ...
    if (/code\s429$/.test(err.message)) {
      Toast.show({ content: '哎呀，当前人数过多，请稍后再试～' });
      return;
    }
    // 超时文案自定义
    if (/^timeout\sof\s\d+ms\sexceeded$/.test(err.message)) {
      Toast.show({ content: '网络不给力，请稍后重试' });
      return;
    }
...
export default function request(
  ...
  return axiosInst({ url, loading, ...options });
}

request.get = (url: string, options: TConfigExpandByAxiosRequestConfig, loading = true) => request(url, options, loading);

request.post = (url: string, options: TConfigExpandByAxiosRequestConfig, loading = true) => {
  options.method = "POST";
  if (options.params) {
    options.data = options.params;
    delete options.params;
  }
  return request(url, options, loading);
};
